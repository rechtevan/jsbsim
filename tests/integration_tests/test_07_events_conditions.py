...
class TestEventsConditions(JSBSimTestCase):
    ...
    def _create_simple_script(
        self, aircraft="ball", initialize="reset00", end_time=10.0, dt=0.00833333
    ):
        """
    ...
    def test_time_based_event(self):
        ...
        self.assertEqual(triggered, 0.0, "Event should not trigger before t=2.0")
        ...
        triggered = fdm["test/time-event-triggered"]
        self.assertEqual(triggered, 1.0, "Event should have triggered at t=2.0")

        # Verify event only triggered once (property still 1.0)
        ExecuteUntil(fdm, 4.0)
        triggered = fdm["test/time-event-triggered"]
        self.assertEqual(triggered, 1.0, "Event should only trigger once")
    ...
    def test_condition_based_event(self):
        ...
        # Initialize a test property that we can control
        init_prop = et.SubElement(run, "property")
        init_prop.set("value", "50.0")
        init_prop.text = "test/speed-value"

        # Add event that triggers when test property exceeds 60
        event = et.SubElement(run, "event")
        event.set("name", "Speed threshold event")

        condition = et.SubElement(event, "condition")
        condition.text = "test/speed-value > 60.0"
        ...
        # Initially at 50 - event should not trigger
        for _ in range(10):
            fdm.run()

        speed_value = fdm["test/speed-value"]
        triggered = fdm["test/speed-event-triggered"]
        self.assertLess(speed_value, 60.0, "Initial speed should be < 60")
        self.assertEqual(triggered, 0.0, "Event should not trigger at low speed")

        # Increase test property above threshold
        fdm["test/speed-value"] = 65.0

        # Run simulation to allow event system to check conditions
        for _ in range(50):
            fdm.run()

        # Verify event triggered
        triggered = fdm["test/speed-event-triggered"]
        self.assertEqual(triggered, 1.0, "Event should trigger when speed exceeds 60")
    ...
    def test_event_with_notify(self):
        ...
        self.assertEqual(triggered, 1.0, "Event with notify should have triggered")
    ...
    def test_persistent_event(self):
        ...
        # Use continuous="true" to fire every frame while condition is true
        event = et.SubElement(run, "event")
        event.set("name", "Persistent event")
        event.set("continuous", "true")

        # Condition true between t=2.0 and t=4.0
        condition = et.SubElement(event, "condition")
        condition.text = "simulation/sim-time-sec >= 2.0\nsimulation/sim-time-sec lt 4.0"

        # For continuous events, we need to verify they fire every frame
        # FG_DELTA with continuous doesn't accumulate - it sets to (original + delta) each frame
        # So we'll use a simple set to verify the event fires continuously
        set_prop = et.SubElement(event, "set")
        set_prop.set("name", "test/trigger-count")
        set_prop.set("value", "1.0")  # Set to 1.0 each frame while condition is true
        # Don't use FG_DELTA - just set the value to verify continuous execution
        ...
        self.assertEqual(count_before, 0.0, "Counter should be 0 before event triggers")

        # During event window (t=3.0) - continuous event should be firing
        # The value should be set to 1.0 (not accumulating, just set each frame)
        ExecuteUntil(fdm, 3.0)
        count_during = fdm["test/trigger-count"]
        self.assertEqual(
            count_during, 1.0, "Continuous event should set value to 1.0 while condition is true"
        )

        # Continue in event window (t=3.5) - should still be 1.0
        ExecuteUntil(fdm, 3.5)
        count_continued = fdm["test/trigger-count"]
        self.assertEqual(
            count_continued, 1.0, "Continuous event should maintain value while condition is true"
        )
        ...
        self.assertEqual(
            count_final, count_after, "Counter should stop increasing after condition becomes false"
        )
    ...
    def test_oneshot_event_explicit(self):
        ...
        # Before trigger
        ExecuteUntil(fdm, 1.5)
        self.assertEqual(fdm["test/oneshot-count"], 0.0, "Counter should be 0 before trigger")

        # Just after trigger
        ExecuteUntil(fdm, 2.5)
        self.assertEqual(
            fdm["test/oneshot-count"], 1.0, "Counter should be 1 after one-shot trigger"
        )

        # Much later, condition still true
        ExecuteUntil(fdm, 6.0)
        self.assertEqual(
            fdm["test/oneshot-count"], 1.0, "Counter should still be 1 (no re-trigger)"
        )
    ...
    def test_compound_condition_and(self):
        ...
        ExecuteUntil(fdm, 2.5)
        self.assertEqual(
            fdm["test/and-event-triggered"], 1.0, "Event should trigger when all conditions true"
        )
    ...
    def test_compound_condition_or(self):
        ...
        # JSBSim uses logic="OR" on condition element, not an <or> tag
        event = et.SubElement(run, "event")
        event.set("name", "OR condition event")

        condition = et.SubElement(event, "condition")
        condition.set("logic", "OR")
        # Multiple condition lines - any one being true triggers the event
        condition.text = "test/value-a > 50.0\ntest/value-b > 15.0"
        ...
        self.assertEqual(triggered, 1.0, "Event should trigger when one OR condition is true")
    ...
    def test_comparison_operators(self):
        ...
        self.assertEqual(fdm["test/gt-result"], 1.0, "50.0 > 40.0 should be true")
        self.assertEqual(fdm["test/lt-result"], 1.0, "50.0 < 60.0 should be true")
        self.assertEqual(fdm["test/ge-result"], 1.0, "50.0 >= 50.0 should be true")
        self.assertEqual(fdm["test/le-result"], 1.0, "50.0 <= 50.0 should be true")
        self.assertEqual(fdm["test/eq-result"], 1.0, "50.0 == 50.0 should be true")
        self.assertEqual(fdm["test/ne-result"], 1.0, "50.0 != 40.0 should be true")
    ...
    def test_event_timing_accuracy(self):
        ...
        # Create all properties before running
        fdm.set_property_value("test/time-1", 0.0)
        ...
        self.assertEqual(fdm["test/trigger-time-1"], 0.0, "Event should not trigger before t=2.00")

        # Check at t=2.01 (just after first event)
        ExecuteUntil(fdm, 2.01)
        self.assertEqual(fdm["test/trigger-time-1"], 1.0, "Event should have triggered by t=2.01")

        # Check at t=4.49 (just before second event)
        ExecuteUntil(fdm, 4.49)
        self.assertEqual(fdm["test/trigger-time-2"], 0.0, "Event should not trigger before t=4.50")

        # Check at t=4.51 (just after second event)
        ExecuteUntil(fdm, 4.51)
        self.assertEqual(fdm["test/trigger-time-2"], 1.0, "Event should have triggered by t=4.51")
