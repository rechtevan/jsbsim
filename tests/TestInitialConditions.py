...
import pandas as pd
from JSBSim_utils import CopyAircraftDef, ExecuteUntil, JSBSimTestCase, RunTest, append_xml
...
class TestInitialConditions(JSBSimTestCase):
    ...
    def test_initial_conditions_v1(self):
        ...
        vars = [
            ...
            },
            {
                ...
                "unit": convtodeg,
                "default_unit": "RAD",
                "ic_prop": "ic/long-gc-deg",
                "prop": "position/long-gc-deg",
            ...
            },
            {
                ...
                "unit": convtodeg,
                "default_unit": "RAD",
                "ic_prop": "ic/theta-deg",
                "prop": "attitude/theta-deg",
            ...
            },
            {
                ...
                "unit": convtodeg,
                "default_unit": "RAD",
                "ic_prop": "ic/psi-true-deg",
                "prop": "attitude/psi-deg",
                "CSV_header": "Psi (deg)",
            },
            {
                "tag": "elevation",
        ...
        for s in self.script_list(("ZLT-NT-moored-1.xml", "737_cruise_steady_turn_simplex.xml")):
    ...
    def LoadScript(self, tree, script_path, prop_output_to_CSV=[]):
        ...
        for element in list(tree.getroot()):
            if "file" in element.keys():
                ...
                name_with_path = os.path.join(os.path.dirname(script_path), name)
        ...
        fdm = self.create_fdm()
        self.assertTrue(fdm.load_script(f), msg="Failed to load script %s" % (f,))
        fdm.run_ic()
    ...
    def test_geod_position_from_init_file_v2(self):
        for s in self.script_list(("ZLT-NT-moored-1.xml", "737_cruise_steady_turn_simplex.xml")):
            ...
            self.CheckICValues(self.GetVariables(), "script %s" % (f,), fdm, position_tag)
    ...
    def test_initial_latitude(self):
        ...
        for v in ("", "_v2"):
            IC_file = self.sandbox.path_to_jsbsim_file("aircraft", "ball", "reset00" + v + ".xml")
            ...
            for i in range(8):
                for latitude_pos in range(4):
                    ...
                    latitude_tag.text = str(latitude_pos * 30.0)
                    ...
                    self.CheckICValues(self.GetVariables(), "IC%d" % (i,), fdm, position_tag)
    ...
    def testNorthPoleInitialization(self):
        ...
        fdm.run_ic()
        self.assertAlmostEqual(fdm["ic/lat-gc-deg"], 90.0)
