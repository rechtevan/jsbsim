...
class TestLGearSteer(JSBSimTestCase):
    ...
    def steerType(self, hasSteerPosDeg, hasSteeringAngle, hasCastered):
        ...
        self.assertTrue(pm.hasNode("gear/unit/steering-angle-deg") == hasSteeringAngle)
    ...
    def isCastered(self):
        ...
        fdm["fcs/steer-cmd-norm"] = 1.0
        fdm.run()
        self.assertEqual(fdm["gear/unit/steering-angle-deg"], float(self.max_steer_tag.text))
        # self.assertEqual(fdm['fcs/steer-pos-deg'],
        #                  fdm['gear/unit/steering-angle-deg'])
    ...
    def isSteered(self):
        ...
        fdm["fcs/steer-cmd-norm"] = 0.5
        fdm.run()
        self.assertEqual(fdm["fcs/steer-pos-deg"], 0.5 * float(self.max_steer_tag.text))

    def test_steer_type(self):
        self.script_path = self.sandbox.path_to_jsbsim_file("scripts", "c1721.xml")
