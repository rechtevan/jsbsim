...
import numpy as np
from JSBSim_utils import CopyAircraftDef, ExecuteUntil, JSBSimTestCase, RunTest
...
class TestAccelerometer(JSBSimTestCase):
    ...
    def testOrbit(self):
        ...
        while fdm.run():
            ...
            self.assertAlmostEqual(fdm["accelerations/a-pilot-x-ft_sec2"], 0.0, delta=1e-8)
            self.assertAlmostEqual(fdm["accelerations/a-pilot-y-ft_sec2"], 0.0, delta=1e-8)
            self.assertAlmostEqual(fdm["accelerations/a-pilot-z-ft_sec2"], 0.0, delta=1e-8)
    ...
    def testSteadyFlight(self):
        ...
        uvw = np.asarray(fdm.get_propagate().get_uvw().T).flatten()
        Omega = omega * np.array(
            [
                math.cos(pitch - geodLat),
        ...
        fa = np.array(
            [
                (fc * math.sin(geodLat - pitch) + g * math.sin(pitch - deviation)),
                fa_yz * math.sin(roll),
                fa_yz * math.cos(roll),
            ]
    ...
    def testSpinningBodyOnOrbit(self):
        ...
        cgy_ft = fdm["inertia/cg-y-in"] / 12.0

        self.assertAlmostEqual(fdm["accelerations/a-pilot-x-ft_sec2"], fax, delta=1e-8)
        self.assertAlmostEqual(fdm["accelerations/a-pilot-y-ft_sec2"], fay, delta=1e-8)
        self.assertAlmostEqual(fdm["accelerations/a-pilot-z-ft_sec2"], faz, delta=1e-8)
        ...
        while fdm.run():
            ...
            if t > 30:
                if not stuck:
                    ...
                    self.assertAlmostEqual(fdm["fcs/accelerometer/Z"], last_output)
