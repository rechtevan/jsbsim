...
import numpy as np
from JSBSim_utils import CopyAircraftDef, CreateFDM, JSBSimTestCase, RunTest
...
class TestExternalReactions(JSBSimTestCase):
    def getLeverArm(self, fdm, name):
        lax = fdm["external_reactions/" + name + "/location-x-in"] - fdm["inertia/cg-x-in"]
        lay = fdm["external_reactions/" + name + "/location-y-in"] - fdm["inertia/cg-y-in"]
        laz = fdm["external_reactions/" + name + "/location-z-in"] - fdm["inertia/cg-z-in"]
    ...
    def test_wind_frame(self):
        script_path = self.sandbox.path_to_jsbsim_file("scripts", "ball_chute.xml")
        fdm = CreateFDM(self.sandbox)

        fdm.load_script(script_path)
        fdm.run_ic()

        self.assertAlmostEqual(fdm["external_reactions/parachute/location-x-in"], 12.0)
        self.assertAlmostEqual(fdm["external_reactions/parachute/location-y-in"], 0.0)
        self.assertAlmostEqual(fdm["external_reactions/parachute/location-z-in"], 0.0)
        ...
        while fdm.run():
            ...
            f = Tw2b * np.array([[-1.0], [0.0], [0.0]]) * mag
            ...
            m = np.cross(self.getLeverArm(fdm, "parachute"), np.array([f[0, 0], f[1, 0], f[2, 0]]))
    ...
    def test_body_frame(self):
        ...
        self.assertAlmostEqual(fdm["external_reactions/pushback/location-x-in"], -2.98081)
        self.assertAlmostEqual(fdm["external_reactions/pushback/location-y-in"], 0.0)
        self.assertAlmostEqual(fdm["external_reactions/pushback/location-z-in"], -1.9683)
        ...
        self.assertAlmostEqual(fdm["external_reactions/hook/location-x-in"], 100.669)
        self.assertAlmostEqual(fdm["external_reactions/hook/location-y-in"], 0.0)
        self.assertAlmostEqual(fdm["external_reactions/hook/location-z-in"], -28.818)
        dx = -0.9995
        dz = 0.01
        ...
        self.assertAlmostEqual(fdm["external_reactions/pushback/magnitude"], 0.05)
        ...
        m = np.cross(self.getLeverArm(fdm, "pushback"), fpb) + np.cross(
            self.getLeverArm(fdm, "hook"), fhook
        )
        self.assertAlmostEqual(fdm["moments/l-external-lbsft"], m[0])
        self.assertAlmostEqual(fdm["moments/m-external-lbsft"], m[1])
        self.assertAlmostEqual(fdm["moments/n-external-lbsft"], m[2])
    ...
    def test_moment(self):
        ...
        while fdm.run():
            ...
            m = -3.5 * Tw2b * np.array(mDir).reshape(-1, 1)
            fm = np.cross(self.getLeverArm(fdm, "parachute"), np.array([f[0, 0], f[1, 0], f[2, 0]]))
