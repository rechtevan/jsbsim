...
class TestEngineIndexedProps(JSBSimTestCase):
    def testEnginePowerVC(self):
        ...
        fdm.load_script(self.sandbox.path_to_jsbsim_file("scripts", "L4102.xml"))
        fdm.run_ic()
        ...
        while fdm.run():
            self.assertAlmostEqual(
                fdm["propulsion/engine[0]/EnginePowerVC"], fdm["propulsion/engine[1]/EnginePowerVC"]
            )

    @pytest.mark.slow
    @pytest.mark.propulsion
    def testFunctionWithIndexedProps(self):
        tree = et.parse(self.sandbox.path_to_jsbsim_file("engine", "eng_PegasusXc.xml"))
        ...
        fdm = CreateFDM(self.sandbox)
        fdm.set_engine_path(".")
        fdm.load_script(self.sandbox.path_to_jsbsim_file("scripts", "Short_S23_1.xml"))
        fdm.run_ic()
        pm = fdm.get_property_manager()
        ...
        while fdm.run():
            ...
            for i in range(4):
                ...
                self.assertAlmostEqual(
                    fdm["propulsion/engine[%d]/starter-max-power-W" % (i,)], maxPower
                )
    ...
    def testTableWithIndexedVars(self):
        ...
        while fdm.run():
            ...
            for i in range(4):
                ...
                self.assertAlmostEqual(
                    fdm["propulsion/engine[%d]/starter-tabular-data" % (i,)], tabularData
                )
